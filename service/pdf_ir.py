...
def locate_canonical_text(pages, text, spans):
    """Verify text and exact spans against pages joined by derived newlines.

    Pages and spans are their serialized IR dictionaries. Inter-page newlines
    belong to the derived text stream, so they never receive fabricated spans.
    """
    ordered = sorted(pages, key=lambda page: page["pdf_page_index"])
    if not spans or [page["pdf_page_index"] for page in ordered] != list(range(len(ordered))):
        raise CanonicalIRError("Canonical page sequence or spans are incomplete")
    offsets, cursor = {}, 0
    for page in ordered:
        offsets[page["pdf_page_index"]] = cursor
        cursor += len(page["normalized_text"]) + 1
    joined = "\n".join(page["normalized_text"] for page in ordered)
    first = spans[0]
    anchor = offsets[first["pdf_page_index"]] + first["char_start_in_normalized_page"]
    for prefix in range(len(text) - len(text.lstrip("\n")) + 1):
        start = anchor - prefix
        end = start + len(text)
        if start < 0 or joined[start:end] != text:
            continue
        expected = []
        for page in ordered:
            page_start = offsets[page["pdf_page_index"]]
            left, right = max(start, page_start), min(end, page_start + len(page["normalized_text"]))
            if left < right:
                expected.append({"file_id": page["file_id"], "pdf_page_index": page["pdf_page_index"],
        ...
        if expected == spans:
            return start
    raise CanonicalIRError("Chunk content does not reconstruct from its canonical spans")
